    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <button className="relative text-muted-foreground hover:text-foreground transition-colors">
          ...
          {totalItems > 0 && (
            <Badge className="absolute -top-2 -right-3 h-4 w-4 rounded-full p-0 flex items-center justify-center text-[10px] bg-accent text-accent-foreground border-0">
              {totalItems}
            </Badge>
          )}
        </button>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full bg-background border-border">
        <SheetHeader className="flex-shrink-0">
          <SheetTitle className="text-foreground font-display">Carrito</SheetTitle>
          <SheetDescription>
                  ...
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-2 rounded-lg bg-card">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-semibold text-foreground text-sm">{item.price.currencyCode} {parseFloat(item.price.amount).toFixed(2)}</p>
                      </div>
